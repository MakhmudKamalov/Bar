...
$workers = $obj->table('dishes')->where('status',0)->get();
rsort($workers);
      ...
      <div class="table-responsive">
                      <table class="table table-bordered table-md">
                        ...
                        <tr>
                          ...
                          <td><?= $v['name']; ?></td>
                          <td>
                          <a href="./add-ing.php?id_add=<?= $v['id'];  ?>&name=<?= $v['name'];  ?>" class="btn btn-primary btn-action mr-1" data-toggle="tooltip" title="" data-original-title="Edit"><i class="fas fa-pencil-alt"></i></a>
                          <a  href="./del-dishes.php?id_del=<?= $v['id'];  ?>" class="btn btn-danger btn-action" data-toggle="tooltip" title="" data-confirm="Are You Sure?|This action can not be undone. Do you want to continue?" data-confirm-yes="alert('Deleted')" data-original-title="Delete"><i class="fas fa-trash"></i></a>
...
document.getElementById("myForm").addEventListener("submit", function(event) {
  ...
  // Задержка в 3 секунды перед скрытием алерта
  setTimeout(function() {
    // Закрываем алерт (ваш alert не исчезнет, так как браузер блокирует изменение стандартных окон alert)
    // Однако, вы можете использовать другие методы для создания кастомных всплывающих окон
  }, 3000);
});
</script>
</body>
